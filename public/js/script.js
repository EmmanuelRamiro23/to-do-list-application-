...
function agregarTarea() {
    ...
    eliminar.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
...
function confirmarTarea(elemento) {
    if (elemento.classList.contains("bg-orange-500")) {
        elemento.classList.replace("bg-orange-500", "bg-green-500");
        elemento.classList.add("line-through", "text-gray-400");
    }
    else{
        elemento.classList.replace("bg-green-500", "bg-orange-500");
        elemento.classList.remove("line-through","text-black-500");
        elemento.classList.replace("text-gray-400","text-black-500");
    }
}
